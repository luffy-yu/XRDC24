...
namespace DA_Assets.FCU.Model
{
    ...
    public class MainSettings : MonoBehaviourBinder<FigmaConverterUnity>
    {
        ...
        public bool OverrideLineSpacing { get => overrideLineSpacing; set => SetValue(ref overrideLineSpacing, value); }

        [SerializeField] bool downloadMultipleFills1 = false;
        [SerializeField] bool downloadMultipleFills2 = true;
        public bool DownloadMultipleFills 
        { 
            get
            {
                if (monoBeh.UsingAnyProceduralImage() || monoBeh.IsUITK() || monoBeh.IsNova())
                {
                    return downloadMultipleFills1;
                }
                else
                {
                    return downloadMultipleFills2;
                }
            }
            set
            {
                if (monoBeh.UsingAnyProceduralImage() || monoBeh.IsUITK() || monoBeh.IsNova())
                {
                    SetValue(ref downloadMultipleFills1, value);
                }
                else
                {
                    SetValue(ref downloadMultipleFills2, value);
                }
            }
        }

        [SerializeField] bool downloadUnsupportedGradients1 = false;
        [SerializeField] bool downloadUnsupportedGradients2 = true;
        public bool DownloadUnsupportedGradients
        {
            get
            {
                if (monoBeh.UsingAnyProceduralImage() || monoBeh.IsUITK() || monoBeh.IsNova())
                {
                    return downloadUnsupportedGradients1;
                }
                else
                {
                    return downloadUnsupportedGradients2;
                }
            }
            set
            {
                if (monoBeh.UsingAnyProceduralImage() || monoBeh.IsUITK() || monoBeh.IsNova())
                {
                    SetValue(ref downloadUnsupportedGradients1, value);
                }
                else
                {
                    SetValue(ref downloadUnsupportedGradients2, value);
                }
            }
        }
    }
