...
{
    [Serializable]
    public class GraphicHelpers : MonoBehaviourBinder<FigmaConverterUnity>
    {
        ...
        { 
            ...
            if (monoBeh.Settings.MainSettings.DownloadMultipleFills)
            {
            ...
            if (monoBeh.Settings.MainSettings.DownloadUnsupportedGradients)
            {
            ...
            if (monoBeh.IsUGUI())
            {
                if (monoBeh.UsingUnityImage() || monoBeh.UsingRawImage() || monoBeh.UsingSpriteRenderer())
                {
        ...
        public float ToLinearAngle(FObject fobject, List<Vector2> gradientHandlePositions)
        {
            ...
            if (!fobject.CanUseUnityImageWhenUsingAnyProceduralImage(monoBeh) && (monoBeh.UsingMPUIKit() || monoBeh.UsingDttPui()))
            {
