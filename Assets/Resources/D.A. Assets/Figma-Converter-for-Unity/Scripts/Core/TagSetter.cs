    ...
    public class TagSetter : MonoBehaviourBinder<FigmaConverterUnity>
    {
        ...
        private void SetTagsByFigma(FObject parent)
        {
            ...
            for (int i = 0; i < parent.Children.Count; i++)
            {
                ...
                {
                    child.Data.HumanizedTextPrefabName = monoBeh.NameHumanizer.GetHumanizedTextPrefabName(child);
                }
        ...
        private void SetSmartTags(FObject parent)
        {
            ...
            {
                bool isInputFieldTextComponents = (parent.Name.IsInputTextArea() || parent.ContainsTag(FcuTag.InputField) || parent.ContainsTag(FcuTag.PasswordField)) && fobject.ContainsTag(FcuTag.Text);

                if (fobject.Data.IsEmpty && !isInputFieldTextComponents)
                {
                    fobject.Data.TagReason = nameof(fobject.Data.IsEmpty);
                    FcuLogger.Debug($"{nameof(SetSmartTags)} | {fobject.Data.TagReason} | {fobject.Data.NameHierarchy}", FcuLogType.SetTag);
                    continue;
                }
