    ...
    public class SpriteDownloader : MonoBehaviourBinder<FigmaConverterUnity>
    {
            ...
            for (int i = 0; i < fobjects.Count(); i++)
            {
                for (int j = 0; j < idLinks.Count(); j++)
                {
                    if (fobjects[i].Id == idLinks[j].Id)
                    {
                        ...
                        {
                            DALogger.LogError($"Can't get link, please check the following component: {fobjects[i].Data.NameHierarchy}");
        ...
        public IEnumerator DownloadSprites(List<FObject> fobjects)
        {
            ...
            foreach (List<FObject> chunk in needDownload.Split(FcuConfig.Instance.ChunkSizeDownloadSprites))
            {
                ...
                {
                    DownloadSprite(item, images, failedObjects, () => requestCount++);
                }
                ...
                foreach (DownloadedSprite image in images)
                {
                    ...
                    //FcuLogger.Debug($"DownloadSprite | {image.FObject.Data.SpritePath}");
            ...
            {
                List<List<string>> comps = failedObjects.Select(x => x.Data.NameHierarchy).Split(50);
        ...
        private void DownloadSprite(FObject item, ConcurrentBag<DownloadedSprite> images, ConcurrentBag<FObject> failedObjects, Action callback)
        {
            ...
            monoBeh.RequestSender.SendRequest<byte[]>(request, (result) =>
            {
                ...
                {
                    DownloadSprite(item, images, failedObjects, callback);
                    return;
                }
                else
                {
                    ...
                    switch (result.Error.Status)
                    {
                        case 909:
                            ...
                            break;
                        default:
                            DALogger.LogError(FcuLocKey.cant_download_sprite.Localize(item.Data.NameHierarchy, result.Error.Message, result.Error.Status));
                            break;
                    }
                }
